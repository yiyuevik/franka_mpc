...
def export_franka_ode_model():
    ...
    nu = configs.Num_Input       # 7 control inputs
    ...
    root_link = configs.root
    tip_link = configs.tip
    ...
    rot_err = log_SO3_vee(ca.mtimes(SO3_target_from_log(p_sym[3:]).T, Rot))  # theta between desired and actual
    ...
    if configs.Obstacle_Avoidance:
        ...
        o_p = configs.Obstacle_Position  # 3D position of the obstacle
        o_s = configs.Obstacle_Scale      # 3D size (radius) of the obstacle
    ...
    model.cost_y_expr = ca.vertcat(pos - p_sym[:3], u_sym)   # dimension: (3 position + 3 orientation + 7 control)
    model.cost_y_expr_e = ca.vertcat(pos - p_sym[:3])      # dimension:  (3 position + 3 orientation)
    return model
