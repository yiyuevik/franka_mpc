"""Implements Conditional Flow Matcher Training"""
...
def apply_conditioning_from_conditioned_x(x, x_conditioned, conditions, action_dim):
    """
    In OT-based flow matching, we need to condition on the conditions with changed indices.

    Use this condition setter to make Diffuser-style conditioning [(time, state), ...] 
    happy.
    """
    for t, val in conditions.items():
...
class FlowMatcher:
    ...
    def get_flow_matcher(self):
        ...
        elif self.flow_matching_type == 'ot_cfm':
            FM = ExactOptimalTransportConditionalFlowMatcher()
        elif self.flow_matching_type == 'vp_cfm':
            FM = VariancePreservingConditionalFlowMatcher()
        elif self.flow_matching_type == 'sb_cfm':
            FM = SchrodingerBridgeConditionalFlowMatcher()
        else:
            raise ValueError(f"Flow matching type {self.flow_matching_type} not supported") 
        return FM
    ...
    def loss(self, x, cond):
        ...
        if self.flow_matching_type in ['cfm', 'vp_cfm']:
            t, xt, ut = self.FM.sample_location_and_conditional_flow(x0, x, t=None)
            xt = apply_conditioning(xt, cond, self.action_dim)
        elif self.flow_matching_type in ['ot_cfm', 'sb_cfm']:
            y1 = apply_conditioning(x, cond, self.action_dim)
            t, xt, ut, y0, y1 = self.FM.guided_sample_location_and_conditional_flow(
                x0, x, y0=None, y1=y1, t=None # change conditions correspondingly in the OT plan
            )
            xt = apply_conditioning_from_conditioned_x(xt, y1, cond, self.action_dim)
        else:
            raise ValueError(f"Flow matching type {self.flow_matching_type} not supported")
...
class GuidanceMatcher:
    def __init__(
        ...
        scale: float = 1.0,
        guidance_matching_type: str = 'direct',
    ):
        assert not train_z or model_z is not None, "If train z, model_z must be provided"
        
        self.train_z = train_z
        self.model = model
        self.scale = scale
        self.guidance_matching_type = guidance_matching_type
        self.flow_matching_type = flow_matching_type
        self.action_dim = action_dim
        ...
        if model_z is not None:
            ...
            assert self.guidance_matching_type in ['rw']

        if model_v is not None:
            self.model_v = model_v
        else:
            assert self.guidance_matching_type in ['direct'] or self.train_z
    ...
    def loss(self, x, cond, values, eps=1e-8):
        """
        Args:
            x: (B, T, C), normalized
            cond: [ (time, state), ... ]; length T, state (B, C_state)
            values: (B, 1), normalized
        """
        assert x.dim() == 3 and values.dim() == 2 and values.shape[1] == 1

        # sample x0 and xt using x1
        x0 = torch.randn_like(x)
        if self.flow_matching_type in ['cfm', 'vp_cfm']:
            t, xt, ut = self.FM.sample_location_and_conditional_flow(x0, x, t=None)
            xt = apply_conditioning(xt, cond, self.action_dim)
        ...
        # learn z (eq:guidance_matching_loss_z)
        process_z = lambda x: x.squeeze(-1)[:, -1].exp().clamp(min=1e-8)
        if self.train_z:
            z_pred = process_z(self.model_z(xt, t)) # (B, T, 1) -> (B,)  NOTE that self.model == self.model_z if self.train_z == False
            loss = torch.nn.functional.mse_loss(z_pred, torch.exp(values.squeeze(-1) * self.scale)).mean()
            infos = {'loss': loss.mean().item()}
            return loss, infos
           
        # learn g
        if self.guidance_matching_type == 'direct': # eq:guidance_matching_loss_g_1
            zt = process_z(self.model_z(x, t)) # (B, T, 1) -> (B,)
            assert zt.shape == (values.shape[0],) == (x.shape[0],)
            gt = self.model(xt, t) # (B, T, C)
            assert gt.shape == x.shape == ut.shape
            loss = torch.nn.functional.mse_loss(
                gt, 
                ((torch.exp(self.scale * values.squeeze(-1)) + eps) / (zt + eps) - 1).reshape(-1, 1, 1) * ut
            ).mean()
        elif self.guidance_matching_type == 'use_learned_v': # eq:guidance_matching_loss_g_2
            zt = process_z(self.model_z(xt, t)) # (B, T, 1) -> (B,)
            gt = self.model(xt, t) # (B, T, C)
            vt = self.model_v(xt, t) # (B, T, C)
            loss = torch.nn.functional.mse_loss(
                gt + vt, 
                ((torch.exp(self.scale * values.squeeze(-1)) + eps) / (zt + eps)).reshape(-1, 1, 1) * ut
            ).mean()
        elif self.guidance_matching_type == 'rw_use_learned_z': # eq:guidance_matching_loss_g_3
            zt = process_z(self.model_z(xt, t)) # (B, T, 1) -> (B,)
            gt = self.model(xt, t) # (B, T, C)
            vt = self.model_v(xt, t) # (B, T, C)
            loss = (
                (gt + vt - ut).square() * (
                    ((torch.exp(self.scale * values.squeeze(-1)) + eps) / (zt + eps)).reshape(-1, 1, 1) # (B,) -> (B, 1, 1)
                )
            ).mean()
        elif self.guidance_matching_type == 'rw': # eq:guidance_matching_loss_g_4
            gt = self.model(xt, t) # (B, T, C)
            vt = self.model_v(xt, t) # (B, T, C)
            loss = (
                (gt + vt - ut).square() * (
                    torch.exp(self.scale * values.squeeze(-1)).reshape(-1, 1, 1) # (B,) -> (B, 1, 1)
                )
            ).mean()
        else:
            raise ValueError(f"Guidance matching type {self.guidance_matching_type} not supported")
        
        infos = {'loss': loss.mean().item()}
        return loss, infos

############## Flow Matching Paths of Different Flow Matching Methods ##############
...
class SchrodingerBridgeConditionalFlowMatcher(ConditionalFlowMatcher):
    ...
    def guided_sample_location_and_conditional_flow(
        ...
        [1] Improving and Generalizing Flow-Based Generative Models with minibatch optimal transport, Preprint, Tong et al.
        """
        x0, x1, y0, y1 = self.ot_sampler.sample_plan_with_labels(x0, x1, y0, y1) # "guided" merely because the labels need to be reindexed simultaneously with the samples?
        if return_noise:
            t, xt, ut, eps = super().sample_location_and_conditional_flow(x0, x1, t, return_noise)
            return t, xt, ut, y0, y1, eps
        else:
